...
function HeatMap() {
    ...
    const groupDataByMonthWeeks = () => {
        const weeks = []
        let currentWeek = []
        let currentMonth = heatmapData[0].month

        for (let i = 0; i < heatmapData.length; i++) {
            const day = heatmapData[i]
            const nextDay = heatmapData[i + 1]
            const nextMonth = nextDay?.month

            currentWeek.push(day)

            // If:
            // - week is full
            // - OR month changes tomorrow
            // Then close the current week
            if (
                currentWeek.length === 7 ||
                (nextMonth !== undefined && nextMonth !== currentMonth)
            ) {
                weeks.push(currentWeek)
                currentWeek = []
            }

            // If month is changing, insert visual gap
            if (nextMonth !== undefined && nextMonth !== currentMonth) {
                weeks.push([]) // Month gap
    ...
    const weekGroups = groupDataByMonthWeeks()
                    ...
                    <div className="heatmap-grid10">
                        ...
                        <div className="heatmap-weeks10">
                            {weekGroups.map((week, weekIndex) => (
                                <div
                                    key={weekIndex}
                                    className={`heatmap-week10 ${week.length === 0 ? 'month-gap10' : ''}`}
                                >
