...
export default function SearchBar() {
  ...
  const filteredCards = useMemo(() => {
    ...
    if (categoryFilter === "frontend") {
      filtered = filtered.filter((card) =>
        card.originalTags.some((tag) => tag.toLowerCase() === "front end")
      );
    } else if (categoryFilter === "backend") {
      filtered = filtered.filter((card) =>
        card.originalTags.some((tag) => tag.toLowerCase() === "back end")
      );
    }

    // Apply sorting by title (use original title for sorting)
  ...
  }, [searchTerm, sortOrder, categoryFilter, processedCardData]);
